...
public class WebSocket : NSObject, NSStreamDelegate {
    ...
    */
    ...
    public func write(string: String, completion: (() -> ())? = nil) {
        ...
        dequeueWrite(data: string.data(using: NSUTF8StringEncoding)!, code: .TextFrame, writeCompletion: completion)
    }
    ...
    /**
     ...
     - parameter completion: The (optional) completion handler.
     */
    public func write(data: NSData, completion: (() -> ())? = nil) {
        guard isConnected else { return }
    ...
    public func write(ping: NSData, completion: (() -> ())? = nil) {
        guard isConnected else { return }
    ...
    private func initStreamsWithData(data: NSData, _ port: Int) {
        ...
        if ["wss", "https"].contains(url.scheme) {
            inStream.setProperty(NSStreamSocketSecurityLevelNegotiatedSSL as AnyObject, forKey: NSStreamSocketSecurityLevelKey)
            outStream.setProperty(NSStreamSocketSecurityLevelNegotiatedSSL as AnyObject, forKey: NSStreamSocketSecurityLevelKey)
        } else {
            certValidated = true //not a https session, so no need to check SSL pinning
        }
        if voipEnabled {
            inStream.setProperty(NSStreamNetworkServiceTypeVoIP as AnyObject, forKey: NSStreamNetworkServiceType)
            outStream.setProperty(NSStreamNetworkServiceTypeVoIP as AnyObject, forKey: NSStreamNetworkServiceType)
        }
        if selfSignedSSL {
